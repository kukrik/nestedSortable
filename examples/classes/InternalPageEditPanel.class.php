...
class InternalPageEditPanel extends Q\Control\Panel
{
    ...
    public function __construct($objParentObject, $strControlId = null)
    {
        ...
        if ($this->objMenuContent->getSelectedPageId()) {
            ...
            if ($strDoubleRouting->getId()) {
                if ($strDoubleRouting->getContentType() == 8 && $strDoubleRouting->getIsRedirect() == 1) {
                    $this->strDoubleRoutingInfo->Text = '<span style="color: #ff0000;"><strong>' . $this->objMenuContent->getSelectedPage() . '</strong>' . ' | ' . t('Warning, double redirection: ') . '<span style="color: #2593a1;">' . $strDoubleRouting->getRedirectUrl() . '</span></span>';
                } else {
    ...
    protected function createToastr()
    {
        ...
        $this->dlgToastr2->Message = t('<strong>Sorry</strong>, the menu title or internal page redirect must exist!');
    ...
    public function createModals()
    {
        ...
        $this->dlgModal1->Text = t('<p style="line-height: 25px; margin-bottom: -3px;">Kas oled kindel, et soovid selle sisemise lehe suunamise kustutada?</p>');
        ...
        if ($this->txtMenuText->Text && $this->lstSelectedPage->SelectedValue) {
            ...
            if ($this->objMenuContent->getSelectedPageId()) {
                ...
                if ($objRedirectUrl->getId() == $this->objMenuContent->getSelectedPageId() && $objRedirectUrl->getIsRedirect() == 1) {
                    $this->objMenuContent->setRedirectUrl('/' . QString::sanitizeForUrl($objRedirectUrl->getMenuText()));
                } else {
